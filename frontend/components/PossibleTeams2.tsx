...
export const fetchUserTeamOne = async (
  provider: any,
  ...
  let team;
  const prediction = await PredictionContract.getPrediction(address, 1);
  const teamName = ethers.utils.defaultAbiCoder.decode(
    ["string"],
    prediction
  )[0];
  if (teamName === "Qatar") {
    team = await getQatar(provider, address);
  }

  if (teamName === "Ecuador") {
    team = await getEcuador(provider, address);
  }
  ...
  if (teamName === "Netherlands") {
    team = await getNetherlands(provider, address);
  }
  return team;
};

export const fetchUserTeamTwo = async (
  provider: any,
  address: string | undefined
) => {
  const PredictionContract = new Contract(
  ...
  let team;
  const prediction = await PredictionContract.getPrediction(address, 2);
  const teamName = ethers.utils.defaultAbiCoder.decode(
    ["string"],
    prediction
  )[0];
...
export const fetchUserTeamThree = async (
  provider: any,
  address: string | undefined
) => {
  const PredictionContract = new Contract(
  ...
  let team;
  const prediction = await PredictionContract.getPrediction(address, 3);
  const teamName = ethers.utils.defaultAbiCoder.decode(
    ["string"],
    prediction
  )[0];
...
export const fetchUserTeamFour = async (
  provider: any,
  address: string | undefined
) => {
  const PredictionContract = new Contract(
  ...
  let team;
  const prediction = await PredictionContract.getPrediction(address, 4);
  const teamName = ethers.utils.defaultAbiCoder.decode(
    ["string"],
    prediction
  )[0];
...
export const getNetherlands = async (
  provider: any,
  address: string | undefined
) => {
  const MintTeamsContract = new Contract(
...
export const getPredictionOne = async (
  provider: any,
  address: string | undefined
) => {
  const PredictionContract = new Contract(
