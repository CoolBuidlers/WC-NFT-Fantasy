...
contract WorldCupData4 is ChainlinkClient, Ownable {
    ...
    function setAddresses(
        address _fetchTeamsOneAddress,
        address _fetchTeamsTwoAddress,
        address _fetchTeamsThreeAddress,
        address _fetchTeamsFourAddress,
    ...
    function receiveTeamOneStanding() private returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(
            jobId,
            address(this),
            this.receiveTeamOne.selector
        );

        req.add(
            "get",
            "https://soccer.sportmonks.com/api/v2.0/teams/season/18017?api_token=API_KEY"
        );
    ...
    function receiveTeamOne(bytes32 _requestId, string memory _teamId)
        public
    ...
    function receiveTeamTwo(bytes32 _requestId, string memory _teamId)
        public
        recordChainlinkFulfillment(_requestId)
    {
    ...
    function receiveTeamThreeStanding() private returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(
            jobId,
            address(this),
            this.receiveTeamThree.selector
        );
    ...
    function receiveTeamThree(bytes32 _requestId, string memory _teamId)
        public
        recordChainlinkFulfillment(_requestId)
    {
    ...
    function receiveTeamFour(bytes32 _requestId, string memory _teamId)
        public
        recordChainlinkFulfillment(_requestId)
    {
