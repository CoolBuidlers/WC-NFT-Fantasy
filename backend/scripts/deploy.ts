...
async function main() {
  ...
  const randomnumberandround = await randomNumberAndRound.deploy(
    2278,
  ...
  await setaddresses.setTheAddresses(
    prediction.address,
    worldcupdata.address,
    fetchteamsone.address,
    fetchteamstwo.address,
    fetchteamsthree.address,
    fetchteamsfour.address,
    mintteamsone.address,
    mintteamstwo.address,
    evolve.address,
    randomnumberandround.address,
    changeorders.address
  );
}
